...
$FakeSmb        = "\\$_LinuxIP\CorpShare"
$FakeSmbUser    = 'CORP\svc-backup'
$FakeSmbPass    = 'BackupSvc!2024fake'
$CredMgrTarget  = "LegacyGeneric:target=\\$_LinuxIP\CorpShare"
...
'@
    (Get-Content $Tokens.EnvFile -Raw) -replace '192\.168\.1\.10', $_LinuxIP |
        Set-Content $Tokens.EnvFile -Encoding UTF8 -NoNewline
...
CorpIntranet,http://${_LinuxIP}:8081,admin,Welcome1!fake,Employee intranet
Corp VPN,https://vpn.corp.internal,jsmith,Vpn@Pass2024!,Legacy VPN (pre-ZPA)
Legacy FileServer,\\${_LinuxIP}\CorpShare,CORP\svc-backup,BackupSvc!2024fake,Backup service account
Dev MySQL,${_LinuxIP}:3307,devuser,DevDB#2024!,Dev database
...
========================
File location : \\${_WindowsIP}\SharedDocs\IT\corp_vault.kdbx
